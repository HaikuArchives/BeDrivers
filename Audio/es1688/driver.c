...
device_hooks dev_hook[] = {
	...
	{	pcm_open,
		...
		pcm_write,
		NULL,NULL,NULL,NULL
		} /*, there will be midi some day... I hope.
	{
		midi_open,
		midi_close,
		midi_free,
...
/*	check for supported card*/
	while(config_info->get_next_device_info(B_ISA_BUS, &cookie, &info,
		...
		if((iinfo->vendor_id&0x0000ffff) != ESS_CARD_ID_MASK) goto END; // was one byte more
		err=B_OK;
		free(dinfo);
		break;
END:
...
status_t check_hw() {
	status_t err;
	acquire_sl ();
	// the magic stuff - I don't know why, but this makes things work.
	read_io (ENABLE1);	/* ENABLE1 */
	read_io (ENABLE1);	/* ENABLE1 */
	read_io (ENABLE1);	/* ENABLE1 */
	read_io (ENABLE2);	/* ENABLE2 */
	read_io (ENABLE1);	/* ENABLE1 */
	read_io (ENABLE2);	/* ENABLE2 */
	read_io (ENABLE1);	/* ENABLE1 */
	read_io (ENABLE1);	/* ENABLE1 */
